...
<div class="wrap">
	...
	<p>
		<?php esc_html_e( 'The "Post Types to Sync" field on the Settings page lists every public post type registered on this site that supports the REST API — internal block-editor/FSE post types like Patterns, Templates, and Navigation Menus are excluded, since they aren\'t content anyone would sync as an article. Posts and Pages are checked by default; select or deselect post types to control which ones are checked and validated.', 'rest-in-sync' ); ?>
	</p>

	<h2><?php esc_html_e( 'Cron sync checks', 'rest-in-sync' ); ?></h2>
	<p>
		<?php esc_html_e( 'A WP-Cron job runs in the background and periodically compares each syncable post against its counterpart on the live site, so out-of-sync content is flagged automatically instead of only being noticed when you manually check. A post is matched to a remote post by trying its own ID directly first (reliable when this site shares history with the live one, e.g. a clone of its database), then falling back to slug, then a scan for a matching GUID or UUID. ID is tried before slug because slug alone can be ambiguous — for example, WPML can give different language translations of the same article the identical slug, so a slug-only match could resolve to the wrong translation. The first time a match is found, its UUID is written back onto the matched remote post so future checks can match by that shared UUID even if the local slug later changes. Posts flagged out of sync appear on the Sync page, where the "Details" link shows exactly which fields differ.', 'rest-in-sync' ); ?>
	</p>
	<p>
	...
	<p>
		<?php esc_html_e( 'Before comparing content, both sites\' home URLs are treated as interchangeable, so internal links and embedded media URLs — which always differ between this site and the live site — don\'t by themselves cause a post to be flagged out of sync. Whitespace is also stripped entirely before comparing, and JSON\'s optional "\/" slash-escaping is normalized, so formatting differences in things like a Gutenberg block\'s JSON attributes (extra indentation, or one site escaping slashes and the other not) aren\'t treated as real content drift either. None of this affects the comparison\'s outcome shown elsewhere — the Sync Details page always shows each field\'s exact saved value.', 'rest-in-sync' ); ?>
	</p>
	<p>
